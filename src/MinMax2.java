...
// float roundOff =
// BigDecimal.valueOf(randomValue).setScale(3,RoundingMode.HALF_UP).doubleValue();
...
// return roundOff;
	...
	public static void main(String[] args) {
		...
		MinMax2 minMaxGraph = new MinMax2(v);
...
// Generate random edges
		...
		while (count <= e) {
			...
			if (minMaxGraph.setEdge(to, from))
				count++;
...
//int loadToTransfer = loadNode - minLoad + 1;
						int loadToTransfer = (loadNode + minLoad) / 2;
						int TenativeLoad = loadNode - loadToTransfer;
...
// int pos=(element.getKey().intValue()) - 1;
							...
							if (minMaxGraph.Load.get(key.intValue() - 1) + 1 == ((maximum))
									|| minMaxGraph.Load.get(key.intValue() - 1) == loadToTransfer) {
								break;
							}
...
// System.out.println("Load after Min Transfer ******* " + minMaxGraph.Load);
					...
					if (checkForBalance(minMaxGraph.adjacencyList, minMaxGraph.Load))
						break;
...
// System.out.println(key);
							...
							if (minMaxGraph.Load.get(key.intValue() - 1) - 1 == minimum
									|| minMaxGraph.Load.get(key.intValue() - 1) == loadNode) {
								break;
							}
...
// checkForBalance(minMaxGraph.adjacencyList,minMaxGraph.Load);
	...
	private static Map<Integer, Float> findSmax(Map<Integer, List<Integer>> adjacencyList2, List<Float> load2,
			int node) {
...
// System.out.println(sorted);
	...
	private static Map<Integer, Float> findSmin(Map<Integer, List<Integer>> adjacencyList2, List<Float> load2,
			int node) {
...
// System.out.println("size:: "+hashSet.size()+":::: Absloute
// Match"+absoluteMatch);
